...
const addDexAddr = async (data) => {
    console.log("***** inside addDexAddr *****");       
    console.log("data inside addDexAddr : "+ JSON.stringify(data));
    try {
        console.log("***** START *****");        
        ...
        await contract.functions.addDexAddresses(data.address);
        ...
        let retReq = {
         ...
         console.log("error callback request :"+ JSON.stringify(retReq));
          await sendTxData(retReq,
              "/api/v1/webhook/addDex/callback"
          );
