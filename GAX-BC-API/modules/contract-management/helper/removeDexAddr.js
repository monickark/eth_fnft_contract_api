...
const removeDexAddr = async (data) => {
    console.log("***** inside removeDexAddr *****");       
    console.log("data inside removeDexAddr : "+ JSON.stringify(data));
    try {
        console.log("***** START *****");        
        ...
        await contract.functions.removeDexAddresses(data.address);
        ...
        let retReq = {
         ...
         console.log("error callback request :"+ JSON.stringify(retReq));
          await sendTxData(retReq,
              "/api/v1/webhook/removeDex/callback"
          );
